...
class Home extends Component {
  ...
  filterRestaurant(searchData) {
    ...
    this.state.allRestaurantList.forEach((element) => {
      if (
        element.restaurant_name.toLowerCase().includes(searchData.toLowerCase())
      ) {
  ...
  render() {
    ...
    return (
      <div>
        <div
          className={this.state.loggedInSuccessfullyMessageClass}
          id="snackbar"
        >
          Logged in successfully!
        </div>
        <div
          className={this.state.registeredSuccessfullyMessageClass}
          id="snackbar"
        >
        ...
        <Grid container spacing={0}>
          {this.state.restaurantListToDisplay.map((restrauntData) => (
            <Grid key={"grid" + restrauntData.id} item xs={6} sm={6} md={3}>
              <div
                onClick={this.openRestaurantDetailsHandler.bind(
                  this,
                  restrauntData.id
                )}
              >
